...
func nextEnts(r *Raft, s *MemoryStorage) (ents []pb.Entry) {
	...
	//println(r.id," append unstable entries length: ", len(r.RaftLog.unstableEntries()))
...
func TestLeaderElectionOverwriteNewerLogs2AB(t *testing.T) {
	...
	//for i := range n.peers {
	//	sm := n.peers[i].(*Raft)
	//	println("node ", i, " id is ", sm.id, " term is ", sm.Term, " vote is ", sm.Vote )
	//}
	////
...
func TestLogReplication2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for j, x := range tt.network.peers {
			sm := x.(*Raft)
			//println(sm.id, " entries length :", len(sm.RaftLog.entries),
			//	"stable :", sm.RaftLog.stabled, "committed :", sm.RaftLog.committed)
