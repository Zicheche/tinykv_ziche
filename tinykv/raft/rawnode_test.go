...
func TestRawNodeStart2AC(t *testing.T) {
	...
	rawNode.Campaign()
	rd := rawNode.Ready()
	//println(len(rd.Entries), len(rd.CommittedEntries),
	//	rawNode.Raft.RaftLog.committed,
	//	rawNode.Raft.State == StateLeader)
	storage.Append(rd.Entries)
	rawNode.Advance(rd)
	//println(rawNode.Raft.RaftLog.stabled, rawNode.Raft.RaftLog.applied)
	rawNode.Propose([]byte("foo"))
	//println(rawNode.Raft.RaftLog.stabled,
	//	rawNode.Raft.RaftLog.committed,rawNode.Raft.RaftLog.applied)
	rd = rawNode.Ready()
	...
	storage.Append(rd.Entries)
	rawNode.Advance(rd)
	//println("after advance: ",rawNode.Raft.RaftLog.stabled,
	//	rawNode.Raft.RaftLog.applied)
	if rawNode.HasReady() {
