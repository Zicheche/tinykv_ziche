...
func testNonleaderStartElection(t *testing.T, state StateType) {
	...
	//leader := r.State == StateCandidate
	//println(r.Term, r.State, r.electionTimeout, leader)
	for i := 1; i < 2*et; i++ {
		r.tick()
	}
...
func TestLeaderElectionInOneRoundRPC2AA(t *testing.T) {
	tests := []struct {
		...
		{5, map[uint64]bool{2: false, 3: false, 4: false}, StateFollower},
	}
...
func TestFollowerAppendEntries2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		r := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, storage)
		r.becomeFollower(2, 2)
		//println("pre stable =", r.RaftLog.stabled)
		...
		//println("HERE, stable =", r.RaftLog.stabled)
...
func TestLeaderSyncFollowerLog2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		//println("loop ---------------: ", i)
		//println("before: ")
		//println("length of lead: ", len(lead.RaftLog.entries))
		//println("length of follower: ", len(follower.RaftLog.entries))
		...
		//println("leader become leader: ", lead.State == StateLeader)
		//println("length of lead: ", len(lead.RaftLog.entries))
		//println("length of follower: ", len(follower.RaftLog.entries))
...
func commitNoopEntry(r *Raft, s *MemoryStorage) {
	...
	for _, m := range msgs {
		if m.MsgType != pb.MessageType_MsgAppend || len(m.Entries) != 1 || m.Entries[0].Data != nil {
			//println(m.MsgType != pb.MessageType_MsgAppend, len(m.Entries), m.Entries[0]!=nil)
